...
- Rate severity yourself, against the spec and the real threat model, not the reviewer's label. Record a change inline with its reason, `F1 [blocker→minor] VALID — needs a same-user process racing inside a private 0700 dir`, or `F4 [nit→blocker]` for a "style" finding that actually breaks CI. Only the confirmed severity counts for the decision. Reviewers err both ways: one passed tests CI would never collect, another called a same-user race a blocker.
...
- **Root cause outside the scope**: when a VALID finding reproduces a bug that already existed before this slice, or whose fix belongs in a file outside "May modify", do not send it back as REWORK. The executor cannot fix it there, so each round only finds the next call site. Accept the slice if its own work is sound, note the finding, and propose a new slice with the shared entry point in scope. Ask the user.
...
- **VALID blocker or major finding and round < max_rounds (2)** → **REWORK**. Put the VALID findings into the handoff's Rework section, then `round += 1`. Ask the user whether to keep the same executor or move up one tier (give your recommendation). Go to 2.3. After the rework run, if the executor itself reports a remaining failure, or you reproduce one before review, skip the review: the outcome is known, and a review would only spend quota. Confirm directly from your reproduction.
